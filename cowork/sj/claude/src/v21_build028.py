"""V21: submit_028.zip — 카운트별 플래툰 추가 + w 0.20 -> 0.25.

V19 (Val2024, w=0.20, 프로덕션 836.503 대비)
    H0_current   +23.60  단독 745.56  corr 0.8570   <- submit_027
    H2_count_pl  +32.04  단독 761.86  corr 0.8384   <- 카운트별 플래툰 +8.44

V20 세 fold 재검증
    fold      격차      w=0.20    w=0.25    w=0.30
    2022      4.34     +43.51    +50.92    +56.94
    2023   1121.16     +18.09     +3.68    -18.30
    2024     73.51     +31.93    +36.27    +39.15

    새 피처가 2023 을 크게 개선했다 (기존 구성은 w=0.20 에서 +4.58 이었다).
    w 를 제약하던 것이 2023 붕괴였는데 카운트별 플래툰이 그 붕괴를 완화했다.
    V18 에서 cap 으로 억지로 막으려다 실패한 것을 피처가 자연스럽게 해결했다.

    V17 에서 세운 규칙 "세 fold 모두 양수인 최대 w" 를 그대로 적용하면 0.25 다.
    0.30 은 2023 이 -18.30 으로 꺾인다. 기대값도 확인했다 -
    w=0.30 은 0.83*(+2.88) + 0.17*(-21.98) = -1.35 로 음수다.

카운트별 플래툰의 핵심은 2단계 차감이다
    split(p,h,count) = EB(투수 x 타자손 x 카운트군) - EB(투수 x 타자손)
    V8 의 성분별 플래툰(+0.16 기각)은 리그평균만 빼서 전역 플래툰과 중복이었다.
    같은 형태인데 전역까지 빼니 +8.44 가 됐다.

팀 ID 범주형(V19 H1, +1.09)은 싣지 않는다. 2022/2023 미검증이고 CatBoost 를
CPU 로 강제해 학습이 3배 걸린다. 검증된 것만 싣는다.

027 대비 변경: 피처 105 -> 108, 성분 모델 80개 재학습, 카운트 플래툰 룩업 추가, w 0.25.
출력: submit/2026-08-15/submit_028.zip
"""
...
BASE_ZIP = ROOT / "submit" / "2026-08-15" / "submit_027.zip"
OUT_ZIP = ROOT / "submit" / "2026-08-15" / "submit_028.zip"
...
print(f"  피처 {X.shape[1]}개  투수platoon {len(platoon)}행  타자platoon {len(bat_platoon)}행  카운트platoon {len(count_platoon)}행", flush=True)
...
json.dump(spec, open(work / "c28_spec.json", "w"), indent=1, sort_keys=True)
...
C28_RATES = ["asof_pitcher_success_rate", "asof_pitcher_reverse_rate",
             "asof_pitcher_middle_rate", "asof_pitcher_ball_rate",
             "asof_pitcher_strike_rate", "asof_pitcher_fastball_rate",
             "asof_pitcher_breaking_rate", "asof_pitcher_offspeed_rate"]
C28_PREV_S = ["asof_pitcher_prev1_game_success_rate",
              "asof_pitcher_prev3_game_success_rate",
              "asof_pitcher_prev5_game_success_rate"]
C28_PREV_M = ["asof_pitcher_prev1_game_middle_rate",
              "asof_pitcher_prev3_game_middle_rate",
              "asof_pitcher_prev5_game_middle_rate"]
C28_CAT = ["top_bottom", "game_type", "base_state"]
C28_RATE_SPECS = [
...
def component_features(frame, spec, platoon, bat_platoon, count_platoon):
    ...
    for name, rate_col, n_col in C28_RATE_SPECS:
        nn = out[n_col]
    ...
    ps = {c: np.where(np.isnan(out[c]), spec["prev_median"][c], out[c])
          for c in C28_PREV_S + C28_PREV_M}
    out["prev_trend_s"] = ps[C28_PREV_S[0]] - ps[C28_PREV_S[2]]
    out["prev_trend_m"] = ps[C28_PREV_M[0]] - ps[C28_PREV_M[2]]
    out["prev_std_s"] = np.std(np.vstack([ps[c] for c in C28_PREV_S]), axis=0)
    out["prev_std_m"] = np.std(np.vstack([ps[c] for c in C28_PREV_M]), axis=0)
    out["prev_miss_cnt"] = sum(np.isnan(out[c]).astype(float)
                               for c in C28_PREV_S + C28_PREV_M)
    for k, (cs, cm) in enumerate(zip(C28_PREV_S, C28_PREV_M)):
...
def component_blend(test, metadata, prediction):
    ...
    for tag in spec["components"]:
        ...
        for seed in spec["seeds"]:
            ...
            booster.load_model(str(MODEL_DIR / ("c28_%s_x%d.ubj" % (tag, seed))))
            v = booster.predict(dmat)
            total = v if total is None else total + v
            count += 1
        for seed in spec["seeds"]:
            model = CatBoostClassifier()
            model.load_model(str(MODEL_DIR / ("c28_%s_c%d.cbm" % (tag, seed))))
...
metadata["track"] = "reverse20_s0475_comp5_uniform_platoon3"
metadata["version"] = 15
metadata["component_blend"] = {
    "spec_file": "c28_spec.json", "platoon_file": "platoon_2025.csv",
    ...
    "validation_val2024": {
        "base": "submit_021", "base_bss": 836.503, "delta_bss": 36.27,
        ...
        "ablation": {"platoon": 13.75, "outside_split": 2.64,
                     "f_row_weight": 4.07, "batter_platoon": 2.44, "count_platoon": 8.44,
...
for fn in files + ["c28_spec.json", "platoon_2025.csv", "bat_platoon_2025.csv", "count_platoon_2025.csv"]:
    payload[f"model/{fn}"] = (work / fn).read_bytes()
for stale in [n for n in payload
              if "/c27_" in n or "/c25_" in n or "/comp_" in n]:
