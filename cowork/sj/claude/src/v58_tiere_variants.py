...
VARIANTS = {1: ("tier_e", "tier_e"), 2: ("tier_e2_abcd16", "tier_e2_abcd16"),
            3: ("tier_e2_abc16", "tier_e2_abc16"), 4: ("tier_e2_a16", "tier_e2_a16")}


def te_profile(fold, version):
    d_, p_ = VARIANTS[version]
    path = OUTS / d_ / f"{p_}_cutoff{fold}.parquet"
...
ARMS = [("G0_a12_shipped", 1), ("G1_abc16", 3), ("G2_a16", 4)]
t0, rows = time.time(), []
for fold in FOLDS:
    va = season == fold
    ...
    for arm, ver in ARMS:
        ...
        np.save(CACHE / f"v58_{arm}_{fold}.npy", p_ie)
...
res.to_csv(OUT / "v58_tiere_variants.csv", index=False)
piv = res.pivot_table(index="arm", columns="fold", values="dbss")
sol = res.pivot_table(index="arm", columns="fold", values="solo_bss")
print(f"{chr(10)}{'='*66}{chr(10)}G0(=submit_033, a축 12차원) 대비{chr(10)}{'='*66}")
print("ΔBSS")
print(piv.subtract(piv.loc["G0_a12_shipped"], axis=1).round(2).to_string())
print(f"{chr(10)}성분단독")
print(sol.subtract(sol.loc["G0_a12_shipped"], axis=1).round(2).to_string())
print(f"{chr(10)}2024 선별이다. 이기면 fold 2023 으로 확인한다.")
