"""V59: M2 — 플래툰 EB 를 리그 평균이 아니라 '커널 이웃' 쪽으로 수축시킨다.
...
커널 공간
    TrackMan 임베딩이 아니라 메인 데이터 프로파일을 쓴다. TrackMan 은 300구
    이상만 있어서 정작 도움이 필요한 저물량 투수에게 없다.
        최근 실패 구성(m/r/mr/ob/oz, 0.6^거리 가중, 실패 대비 구성비) + 실패율 + 물량
        TrackMan 물리 7종은 있으면 추가(표준화 후 결측 0)
    전부 학습 시즌만으로 만든다.
...
d = pd.DataFrame({"k": pid, "w": sw, "fail": FAIL * sw})
for c in COMPONENTS:
    d[c] = np.nan_to_num(LAB[c]) * sw * (~np.isnan(LAB[c]))
g = d.groupby("k").sum()
n_obs = pd.Series(tr * 1.0).groupby(pid).sum().reindex(g.index).to_numpy()
tot = g["w"].to_numpy()
prof = np.column_stack(
    [g["fail"].to_numpy() / np.maximum(tot, 1e-9)]
    + [g[c].to_numpy() / np.maximum(g["fail"].to_numpy(), 1e-9) for c in COMPONENTS]
    + [np.log1p(n_obs)])
tg = pd.DataFrame(TMV[tr], columns=TM_COLS).assign(k=pid[tr]).groupby("k").mean()
prof = np.column_stack([prof, tg.reindex(g.index).to_numpy()])
lo, hi = np.nanpercentile(prof, [1, 99], axis=0)
prof = np.clip(prof, lo, hi)
Z = np.nan_to_num((prof - np.nanmean(prof, 0)) / (np.nanstd(prof, 0) + 1e-9))
PIDS = g.index.to_numpy()
D2 = cdist(Z, Z, "sqeuclidean")
med = float(np.median(np.sqrt(D2[np.triu_indices_from(D2, 1)])))
print(f"투수 {len(PIDS)}명  프로파일 {Z.shape[1]}차원  중앙 쌍거리 {med:.3f}",
      flush=True)
POS = {p: i for i, p in enumerate(PIDS)}
ROW = np.array([POS[p] for p in pid])
...
def kernel_tables(sigma, keys, axis=None):
    ...
    cols = {"p": ROW[tr], "h": bhand[tr], "y": y_all[tr]}
    if axis is not None:
        cols["a"] = axis[tr]
    ...
    for kk, sub in ctx:
        ...
        keyidx[kk] = (s, n)
    out_eb = np.full(len(df), lg)
    out_mu = np.full(len(df), lg)
    ...
    for kk in S_all:
        ...
        s, n = keyidx[kk]
        eb = (s + K * mu) / (n + K)
        out_eb[mask] = eb[ROW[mask]]
        out_mu[mask] = mu[ROW[mask]]
    return out_eb - out_mu, np.array([0.0])
...
for name, smul, apply_all in ARMS:
    ...
    if smul is not None:
        sig = med * smul
        sp, _ = kernel_tables(sig, None)
        F["k_platoon_split"] = sp
        if apply_all:
            for tag, ax in [("cnt", cnt_b), ("inn", inn_b)]:
                s2, _ = kernel_tables(sig, None, axis=ax)
                F[f"k_{tag}_split"] = s2
    p_ie = line(F.to_numpy(np.float32))
    np.save(CACHE / f"v59_{name}.npy", p_ie)
...
res.to_csv(OUT / "v59_kernel_shrink.csv", index=False)
