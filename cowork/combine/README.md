# cowork/combine — 팀 모델 결합 공용 폴더 (2026-08-29 이후 대체됨)

> ⚠️ **이 폴더는 더 이상 단일 출처가 아니다.** 2026-08-28부터
> [`performance_tracking/`](../../performance_tracking/README.md) 이 모델 등록·
> val 예측·상관계수·결합가중 계산의 공식 위치다 (등록 규칙 4개 + `tools/score_val.py`
> `tools/corr.py`). 여기 있는 `compute_correlation.py` / `compute_blend_weights.py`
> 는 3~4자 결합 초기 단계(8/26~27)에서 쓰던 것으로, 관문시즌(2022 all)이
> 2023 신설 전 구식 규칙이라 **결과 숫자를 새 작업의 기준선으로 쓰지 말 것.**
> 코드 자체(자동 멤버탐색, w*=M^-1A 닫힌형태)는 여전히 참고할 만해서 지우지
> 않고 남겨둔다.

`plan.md` 우선순위 1 "팀 모델 결합"을 위한 공용 작업 공간이었다. 각자 폴더(`cowork/<initial>/`)는
