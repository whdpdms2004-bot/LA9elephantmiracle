"""submission_v10 -- v9(anchor+platoon, 실LB 857.4009) 위에 team_id 범주형
전환만 추가. **실제 제출 결과: Public 892.1204835291 (+34.72)** -- 이번
세션에서 가장 큰 단일 구조변경 성과라 팀 공유용으로 올림.

yn님의 903.37->939.94 여정 문서에서 핵심 원인으로 지목한 것을 내 파이프라인에
이식: pitcher_team_id/batter_team_id가 지금까지 CatBoost 네이티브 범주형
분기를 못 쓰고 숫자로만 처리되고 있었다. 이걸 범주형으로 바꾸는 정직한
3개년 교차검증(fit<val_season)에서 2022 +14.57 / 2023 +2.36 / 2024 +33.92,
전부 양의 방향으로 확인 후 production 반영, 실제로도 큰 폭 개선됨.

한 번에 하나만 바꾼다(AGENTS.md 원칙): BASELINE_CATS에 team_id 2개만 추가,
나머지(feature set, 하이퍼파라미터, 시드 수, 플래툰 lookup 방식)는 v9와
완전히 동일. pitcher_hand/batter_hand는 이번 검증에 없었으므로 손대지 않음.
`pitcher_team_id`/`batter_team_id` 둘 다 고유값 13개뿐이라(팀 수만큼) 저카디널리티
-- 과적합 위험 낮음. 다른 라인에도 그대로 적용해볼 만함(각자 아키텍처에서 정직
검증 먼저 권장 -- 내 아키텍처에서 통했다고 다른 라인에도 자동으로 통하진 않음,
승주님 preprocess_lab 원자들을 내 라인에 이식했을 때 전부 마이너스였던 반대
사례가 있었음).

Feature set: baseline47 + trend6 + platoon_split + platoon_n = 55개 (v9와 동일)
BASELINE_CATS: top_bottom, game_type, base_state, pitcher_team_id, batter_team_id
Model: CatBoost 16-seed, v9와 동일 하이퍼파라미터. 체크포인트 지원.
...
## 실행
    cd cowork/hw
    python train_best_model_v10_teamid.py

정희원 로컬(12코어, CPU) 기준 PHASE1+PHASE2 합쳐 약 164분 걸렸습니다.

## 중간에 꺼져도 괜찮습니다 (체크포인트 지원)
`submission_v10/phase1_progress.jsonl`에 멤버별 결과가 즉시 저장되고,
`model/`에 이미 저장된 파일은 건너뜁니다.
"""
import json
import sys
import time
...
OUT_DIR = Path(__file__).resolve().parent / "submission_v10"
...
KNOWN_V9_BSS_2024 = 723.85  # 참고용 -- validate_teamid_categorical.py 단일시드 기준선(team_id=숫자)
...
def main():
    ...
    feature_cols = baseline_47 + trend_cols + platoon_cols
    ...
    for i, seed in enumerate(CAT_SEEDS, 1):
        ...
        print(f"  [{i}/{len(CAT_SEEDS)} seed={seed}] BSS={bss:8.2f} best_iter={best_iter} "
              f"(v9 team_id=숫자 단일시드 기준선 {KNOWN_V9_BSS_2024})", flush=True)
        preds_p1.append(p)
    ...
    if len(preds_p1) == len(CAT_SEEDS) or skipped == len(CAT_SEEDS):
        ...
        print(f"  [{len(CAT_SEEDS)}-seed 앙상블] BSS={ens_bss:8.2f}  "
              f"v9(team_id=숫자, 실LB 848.44 라인) 대비 참고 비교용")
    ...
    print("다음 단계: script_v10.py 작성 -> 체크리스트 -> zip")
