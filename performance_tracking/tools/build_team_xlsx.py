...
ASSETS = [
    ...
    ("sj3way", "sj", "cowork/sj/val2024_pred.csv",
     "sj 자체", None,
     "챔피언 zip 의 model/sj/ 멤버(submit_045 계열). 팀 결합의 두 축 중 하나. val2022 없음"),
    ("sj3way_nv", "sj 재현", "cowork/sj/new_val/three_way/src/run_way.py --folds 2022,2024",
     "sj 재현 (4way x 900회, strict-forward)", None,
     "★배포본과 **다른 구성**(new_val b계열)이라 826.2 로 약간 낮다. "
     "harness3.py 의 BANNED_FOLD=2022 가 막고 있었는데 그 정책은 이미 철회됐다. "
     "이걸로 §31 의 팀층 규약 fit(2022)->eval(2024) 을 처음 돌릴 수 있었다"),
...
def main():
    ...
    verdict = pd.DataFrame([
        ...
        ("── §31 규약 fit(2022)→eval(2024) (시즌 간 전이) ──", "", "", ""),
        ("cw + sj3way + hw(정직)", "0.90/0.05/0.05", "★최고 930.7",
         "월분할과 어긋난다 — 여기서는 hw 가 가중을 받는다"),
        ("cw + hw(정직)", "0.90 / 0.10", "928.7", "역방향 2476.5 로 이쪽이 더 안정적"),
        ("cw + sj3way", "0.95 / 0.05", "927.2", ""),
        ("cw 단독", "1.00", "921.8", "기준"),
        ("── ★중재 fit(2023)→eval(2024) — 배포 동형 ──", "", "", ""),
        ("cw 단독", "1.00", "921.8", "sj3way·hw·ye 전부 가중 0.00 (퇴화 해)"),
        ("── 역 fit(2024)→eval(2022) ──", "", "", ""),
        ("cw 단독", "1.00", "2479.5", "cw+hw 는 2476.5 로 더 낮다"),
        ("★ 최종 판단", "cw + sj3way", "hw 기각",
         "네 계기 중 셋이 hw 를 배제한다. 편입하는 것은 §31(fit2022) 하나뿐이고 "
         "그 fit 폴드는 구조 단절 이전이다. 단 중재 계기는 fold2023 의 모든 멤버 BSS 가 "
         "음수라 가중이 [1,0,0] 으로 퇴화했다 — 약한 증거로 센다"),
    ], columns=["조합", "가중", "판정", "근거"])
