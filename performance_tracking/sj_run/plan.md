...
### 추가 확인 (2026-08-30) — hw 의 페어드 부트스트랩 CI 로 재판정

hw 가 관문에 붙인 CI 도구(commit 79aa20b)로 다시 쟀습니다. `score_val.py
ye_form1_injected --baseline sj_stdmlp --skip-layout`:

```
2024 all   916.0 -> 915.8  (-0.2)  [-2.4, +1.9]  무증거 (CI 0 포함)
2022 R     668.1 -> 665.2  (-2.9)  [-5.3, -0.7]  유의하게 하락
판정(CI):  무증거 2024 미상승 / 유의 2022 R 하락
```

**결론은 안 바뀌고 오히려 더 단단해졌습니다.** 2024 미상승은 CI 상 잡음과
구분 안 되지만(무증거), **2022 R 하락은 CI가 0을 안 넘어 통계적으로 유의**합니다
— TOL=0.0 의 과탄력 문제(하락으로 오판)가 아니라 진짜 하락입니다. 기각 유지.

---

## 7순위 (ye 추가, 2026-08-30) -- 외부 리포트 기반 피드백 2건 (5순위 GPU 낭비 방지 + 전이율 재검토 요청)

**출처 주의**: 팀원(추정 리더보드 1150 도전 트랙, 소속 불명 -- 인코딩 깨진 채로
전달받아 한글 프로즈는 재구성한 것이라 세부 문구는 확신 못 합니다. 숫자·표·코드는
원문 그대로입니다. **아래 두 건은 검증 요청이지 확정 사실이 아닙니다.**

### 7-1. 5순위 그리드의 절반이 이 파이프라인에서 구조적으로 무효라는 지적

`min_data_in_leaf` -- 이 파이프라인이 쓰는 `grow_policy=SymmetricTree` 아래서는
무시된다고 합니다. `bagging_temperature` -- CPU RMSE 기본 bootstrap 이 MVS 라
무시된다고 합니다. **맞다면 5순위 Group D(min_data_in_leaf)·Group C(bootstrap)는
GPU 돌려도 결과가 안 바뀝니다.** sj님이 5순위 시작하시기 전에, CatBoost 문서나
빠른 단위테스트로 이 두 파라미터가 실제로 무시되는지 먼저 확인 부탁드립니다.
맞으면 Group A(random_strength)·B(border_count)만 남기고 진행해주세요.

### 7-2. "전이율 0.075 법칙"이 기준선 혼용에서 온 착시일 수 있다는 주장

`sj_transfer_law`(results.csv)의 0.075(+1.68/+22.3)는 **카드 계기 기준선(893.7)**
을 썼는데, **등록 계기 기준선(912.8)** 으로 통일하면 927.8-912.8=+15.0 →
9.24/15.0=**0.62** 가 나온다는 주장입니다. 저희 A177(pitcher_futures_share) 실측
전이율도 이 문서에서 0.622 로 계산돼 있어 (2024 all +3.2 → Public 추정 +0.9 언저리),
0.075 보다 0.62 에 훨씬 가깝습니다.

**맞다면 파급력이 큽니다** -- `TEAM_BRIEF.md` §7 이 이 0.075 를 근거로 "피처/표현
탐색 종료"를 권고했는데, 그 전제가 계기 혼용이었다면 결론이 바뀔 수 있습니다.
다만 **저는 이 계산을 직접 재현하지 못했습니다** (원본 예측 배열이 없음) --
sj님/승주님이 각 제출본의 "카드 계기" vs "등록 계기" val 값을 한 표로 다시
정리해서 실제로 0.62 가 나오는지만 확인해주시면 좋겠습니다.
